...
// Reuse VoteableButton component (same as QF)
...
export default function SemiFinal({ winners, onAdvance }: Props) {
  ...
  const toggleWinner = (matchId: string, team: string) => {
    setSfWinners({ ...sfWinners, [matchId]: team });
  ...
  const handleNext = () => {
    ...
    onAdvance(matches.map(m => sfWinners[m.id] as string));
  };
  ...
  return (
    <div style={{ padding: '2rem' }}>
        ...
        {matches.map(m => {
          ...
          return (
            <div key={m.id} style={{ backgroundColor: '#8B0000', padding: '1rem', borderRadius: '8px' }}>
              <h3 style={{ textAlign: 'center', marginBottom: '1rem', color: '#fff' }}>{m.id}</h3>
      ...
      <div style={{ textAlign: 'center' }}>
        <button onClick={handleNext} style={{ padding: '0.75rem 1.5rem', backgroundColor: '#FFD700', color: '#8B0000', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>
          Next Round
        </button>
      </div>
    </div>
